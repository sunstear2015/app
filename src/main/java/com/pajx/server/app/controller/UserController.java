...
public class UserController extends BaseController {
     ...
     * @return json
     */
    @RequestMapping(value = "/api/v1/user/search")
    public
    @ResponseBody
     ...
     * @return json
     */
    @RequestMapping(value = "/api/v1/user/detail")
    public
    @ResponseBody
    Object v1_userDetail(@RequestParam String stuno, @RequestParam String api_key, @RequestParam String pajx_sign, @RequestParam String call_id) {
        JSONObject jsonObject = new JSONObject();
        /*if (StringUtils.isEmpty(pajx_sign)) {
            jsonObject.put("status", false);
            jsonObject.put("message", "参数签名为空");
      ...
      /* String sign = generate_sign(usertype,content);
        ...
        try {
            List userList=userService.getSearchUser(stuno);
            List<JSONObject> listobj = new ArrayList<JSONObject>();
            List<JSONObject> listobjfam = new ArrayList<JSONObject>();
            String stuid="";
            if (userList != null && !userList.isEmpty()) {
                for (int i = 0; i < userList.size(); i++) {
                    ...
                    jsonObject1.put("STU_NO", o[0]);
                    jsonObject1.put("SCL_NAME", o[1]);
                    jsonObject1.put("GRA_NAME", o[2]);
                    jsonObject1.put("CLS_NAME", o[3]);
                    jsonObject1.put("STU_NAME", o[4]);
                    jsonObject1.put("STU_TYPE", o[5]);
                    stuid=o[6].toString();
            ...
            if (StringUtils.isNotEmpty(stuid)) {
                ...
                if (famList != null && !famList.isEmpty()) {
                    for (int i = 0; i < famList.size(); i++) {
                        ...
                        jsonObject1.put("FAM_RELATION", o[0]);
                        jsonObject1.put("FAM_PHONE", o[1]);
                        jsonObject1.put("BIZ_STATUS_FLAG",o[3]);
                        jsonObject1.put("BIZ_PRICE_TYPE",biz);
                        jsonObject1.put("LAST_DEAL_TIME", o[4]);
                        listobjfam.add(jsonObject1);
                    }
                }
            }
            jsonObject.put("STUDENT",listobj);
            jsonObject.put("FAMILY",listobjfam);
            jsonObject.put("status", true);
            jsonObject.put("message", "获取接口成功");
            return jsonObject;
        } catch (Exception e) {
